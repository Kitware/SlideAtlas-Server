...
//==============================================================================
// Mouse down defined the the first point.
// Drag defines the second point.


// The rectangle has just been created and is following the mouse.
// I can probably merge this state with drag. (mouse up vs down though)
var RECTANGLE_WIDGET_NEW = 0;
var RECTANGLE_WIDGET_DRAG = 1; // The whole arrow is being dragged.
var RECTANGLE_WIDGET_DRAG_RADIUS = 2;
var RECTANGLE_WIDGET_WAITING = 3; // The normal (resting) state.
var RECTANGLE_WIDGET_ACTIVE = 4; // Mouse is over the widget and it is receiving events.
var RECTANGLE_WIDGET_PROPERTIES_DIALOG = 5; // Properties dialog is up
...
function RectangleWidget (viewer, newFlag) {
    ...
    this.Dialog.Title.text('Rectangle Annotation Editor');
    ...
    if (localStorage.RectangleWidgetDefaults) {
        var defaults = JSON.parse(localStorage.RectangleWidgetDefaults);
        if (defaults.Color) {
...
RectangleWidget.prototype.PasteCallback = function(data, mouseWorldPt) {
  this.Load(data);
...
RectangleWidget.prototype.HandleMouseWheel = function(event) {
...
RectangleWidget.prototype.HandleKeyPress = function(keyCode, shift) {
  ...
  if (this.State == RECTANGLE_WIDGET_PROPERTIES_DIALOG) {
      return false;
  }
  ...
  if (event.keyCode == 67 && event.ctrlKey) {
    ...
    var clip = {Type:"RectangleWidget", Data: this.Serialize()};
...
RectangleWidget.prototype.HandleMouseUp = function(event) {
    if ( this.State == RECTANGLE_WIDGET_DRAG || this.State == RECTANGLE_WIDGET_DRAG_RADIUS) {
...
RectangleWidget.prototype.HandleMouseMove = function(event) {
    ...
    if (event.which == 0 && this.State == RECTANGLE_WIDGET_ACTIVE) {
        this.CheckActive(event);
        return;
    }
    
    if (this.State == RECTANGLE_WIDGET_NEW || this.State == RECTANGLE_WIDGET_DRAG) {
    ...
    if (this.State == RECTANGLE_WIDGET_DRAG_RADIUS) {
...
RectangleWidget.prototype.CheckActive = function(event) {
  ...
  // change dx and dy to vector from center of rectangle.
  if (this.FixedSize) {
  ...
  if (this.Shape.FillColor == undefined) { // Rectangle
...
RectangleWidget.prototype.PlacePopup = function () {
  // Compute the location for the pop up and show it.
...
var RECTANGLE_WIDGET_DIALOG_SELF;
RectangleWidget.prototype.ShowPropertiesDialog = function () {
...
RectangleWidget.prototype.DialogApplyCallback = function() {
    ...
    localStorage.RectangleWidgetDefaults = JSON.stringify({Color: hexcolor, LineWidth: this.Shape.LineWidth});
    if (SA.NotesWidget) {SA.NotesWidget.MarkAsModified();} // hack
}
