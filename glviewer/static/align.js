...
(function () {
    ...
    SA.PermuteBounds = function(bds, axis, direction) {
        if (direction < 0) {
            axis = axis << 1;
            var tmp = bds[axis];
            bds[axis] = -bds[axis+1];
    ...
    function SortAndFilterContours(contours, firstContour) {
        ...
        SA.PermuteBounds(lastBds, axis0, direction0);
        SA.PermuteBounds(lastBds, axis1, direction1);
        var bestBds = false;
        while (true) {
            var bestIdx = -1;
            for (var i = 0; i < contours.length; ++i) {
                var bds = contours[i].GetBounds();
                SA.PermuteBounds(bds, axis0, direction0);
                SA.PermuteBounds(bds, axis1, direction1);
    ...
    function SortContours(contours, axis0, axis1) {
        ...
        function lessThanContours(c1,c2) {
            ...
            SA.PermuteBounds(bds1, axis0, direction0);
            SA.PermuteBounds(bds1, axis1, direction1);
            var bds2 = c2.GetBounds();
            SA.PermuteBounds(bds2, axis0, direction0);
            SA.PermuteBounds(bds2, axis1, direction1);
    ...
    // Take a list of image points and make a viewer annotation out of them.
    Contour.prototype.MakeStackSectionWidget = function(view) {
        // Make an annotation out of the points.
        // Create a widget.
        var w = new SA.StackSectionWidget();
        w.Shapes.push(this.MakePolyline([0,1,0], view));
    ...
    function testSegment() {
        if (window.SA) {
            ...
            SA.VIEWERS[0].MainView.Canvas.mouseup(function ()
                                                  {HandleMouseUp(event);});
        }
    }
    ...
    SA.SeedIsoContour = function(data, x0,y0, x1,y1, threshold) {
        this.Bounds = [0,-1,0,-1];
    ...
    SA.ComputeIntensityHistogram = function(data, ignoreWhite) {
    ...
    function ComputeContourBounds(contourObj) {
        var contour = contourObj.Points;
    ...
    function AlignContours(contour1, contour2) {
        ...
        var distMap = new SA.DistanceMap(bds1, 2);
    ...
    function DeformableAlignContours(contour1, contour2, rigid) {
        ...
        var distMap = new SA.DistanceMap(bds1, spacing);
        distMap.AddContour(contour1);
        distMap.Update();
    ...
    SA.DeformableAlignViewers = function(keepExistingFiducials) {
        ...
        window.setTimeout(
            function () {
                ...
                var histogram1 = SA.ComputeIntensityHistogram(data1, true);
                var threshold1 = SA.PickThreshold(histogram1);
                var contour1 = LongestContour(data1, threshold1);

                viewer = SA.VIEWERS[1];
                var data2 = viewer.MainView.GetImageData();
                SmoothDataAlphaRGB(data2, 2);
                var histogram2 = SA.ComputeIntensityHistogram(data2, true);
                var threshold2 = SA.PickThreshold(histogram2);
                ...
                for (var i = 2; i < originalContour2.Length(); i += skip) {
                    ...
                    var cor = new SA.PairCorrelation();
                    cor.SetPoint0(pt1);
                    cor.SetPoint1(pt2);
    ...
    // intensity range.
    SA.PickThreshold = function(hist) {
    ...
    function PickThresholdContaining(hist, val) {
        ...
        for (var idx = val; idx < hist.length-10; ++idx) {
    ...
    function intensityHistogram(viewer, color, min, max) {
        ...
        var data1 = viewer.MainView.GetImageData();
        var histogram1 = SA.ComputeIntensityHistogram(data1);
    ...
    function testAlignTranslationPixelMean() {
        ...
        var histogram2 = SA.ComputeIntensityHistogram(data2);
        var threshold2 = SA.PickThreshold(histogram2);
    ...
    SA.testAlignTranslation = function(debug) {
        var viewer1 = SA.VIEWERS[0];
    ...
    function testDistanceMapContour() {
        ...
        var bds1 = ComputeContourBounds(contour1);
        var distMap = new SA.DistanceMap(bds1, 1);
        distMap.AddContour(contour1);
        distMap.Update();
    ...
    function FindSectionContour (data, center, area) {
        ...
        var contours = SA.GetHagFishContours(data, threshold, min, max);
    ...
    function FindSectionContours(data, val) {
        ...
        SmoothDataAlphaRGB(data, smooth);
        var histogram = SA.ComputeIntensityHistogram(data, true);
        var threshold = SA.PickThresholdContaining(histogram, val);
        var contours = SA.GetHagFishContours(data, threshold, min, max);
    ...
    SA.GetHagFishContours = function(data, threshold, areaMin, areaMax) {
    ...
    function findHagFishSections(smooth, min, max) {
        ...
        var histogram1 = SA.ComputeIntensityHistogram(data1, true);
        var threshold1 = SA.PickThreshold(histogram1);
        var contours = SA.GetHagFishContours(data1, threshold1, min, max);
