...
BinaryBuffer = function(bigEndian, buffer){
...
BinaryBuffer.prototype.readBits = function(start, length){
  if(start < 0 || length <= 0)
    return 0;
...
BinaryBuffer.prototype.setBuffer = function(data){
  if(data){
    this.buffer = data;
    if (this.bigEndian) {
      for (var s = 0, e = data.length - 1; s<e; ++s, --e) {
        var tmp = this.buffer[e];
	this.buffer[e] = this.buffer[s];
	this.buffer[s] = tmp;
      }
    }  
  }
...
BinaryBuffer.prototype.hasNeededBits = function(neededBits){
  return this.buffer.length >= -(-neededBits >> 3);
}

BinaryBuffer.prototype.checkBuffer = function(neededBits){
...
BinaryParser.prototype.decodeInt = function(data, bits, signed){
    var b = new BinaryBuffer(this.bigEndian, data); 
...
BinaryParser.prototype.decodeFloat = function(data, precisionBits, exponentBits){
  var b = ((b = new BinaryBuffer(this.bigEndian, data)).checkBuffer(precisionBits + exponentBits + 1), b),
